<script lang="ts">
	...
	const handleJiraOAuthMessage = async (event: MessageEvent) => {
		...
		if (event.data?.type === 'atlassian_connected' && event.data?.layer === 'jira') {
			...
			const jiraDataSource = dataSources.find(
				(ds) => ds.action === 'atlassian' && ds.layer === 'jira'
			);
			if (jiraDataSource) {
	...
	const checkSocketTimeout = async () => {
		...
		console.log(
			`🕐 socket: ${Math.round(timeSinceActivity / 1000)}s old, ${syncingSources.length} sources syncing`
		);
	...
	async function fetchEmbeddingStatus() {
		...
		try {
			...
			const isEmptyResponse =
				newEmbeddingStatus && Array.isArray(newEmbeddingStatus) && newEmbeddingStatus.length === 0;

			if (hasEmbeddingSources && isEmptyResponse) {
				consecutiveEmptyResponses++;
				console.log(
					`🧠 Empty embedding response ${consecutiveEmptyResponses}/4 (${dataSources.filter((ds) => ds.sync_status === 'embedding').length} sources embedding)`
				);
					...
					if (dataSource) {
						...
						try {
							...
							if (isEmbeddingComplete) {
								console.log(
									`✅ Embedding completed for ${dataSource.name} - transitioning to synced`
								);
							}
							...
							if (
								dataSource &&
								(dataSource.sync_status === 'syncing' || dataSource.sync_status === 'embedding') &&
								getActiveView(dataSource) === 'sync'
							) {
	...
	$: {
		dataSources.forEach((dataSource) => {
			...
			if (
				previousStatus === 'embedding' &&
				currentStatus === 'synced' &&
				getActiveView(dataSource) === 'embedding'
			) {
	...
	const handleSync = async (dataSource: DataSource) => {
		...
		try {
			...
			if ($user?.id) {
				...
				const dataSourceName = formatDataSourceName(
					dataSource.action ?? '',
					dataSource.layer ?? ''
				);
	...
	const handleMineralSync = async (dataSource: DataSource) => {
		if (dataSource.sync_status === 'unsynced' || dataSource.sync_status === 'deleted') {
		...
		dataSources = dataSources.map((ds) => {
			if (ds.name === sourceData.source) {
				return {
					...
					files_total:
						sourceData.files_total !== undefined ? sourceData.files_total : ds.files_total,
	...
	const updateSync = async (action: string, layer: string) => {
		...
		try {
			...
			if (!syncDetails) {
				console.error(
					'Sync failed - no details returned. This usually means the sync is already in progress or there was an authentication error.'
				);
				return;
			}
	...
	const isProcessing = (dataSource: DataSource) => {
		...
		return (
			...
			authCheckInProgress.has(actionKey) ||
			dataSource.sync_status === 'syncing' ||
			dataSource.sync_status === 'deleting'
		);
	};
...
>
	<div class="text-sm text-gray-500">
		{$i18n.t('This will delete the')} <span class="font-semibold">{selectedDataSource?.name}</span>
		{$i18n.t(
			'files from the AI system. You may resync the data source anytime after the delete process completes.'
		)}
	</div>
</ConfirmDialog>
		...
		{#if filteredItems.length > 0}
			...
			<div class="hidden lg:block overflow-x-auto">
				<table class="w-full table-fixed data-sources-table">
					<thead>
						<tr>
							<th class="text-left py-3 px-4 font-medium text-gray-700 dark:text-gray-300 col-name">
							</th>
							<th
								class="text-left py-3 px-4 font-medium text-gray-700 dark:text-gray-300 col-status"
							>
								<!-- Status column - no header text -->
							</th>
							<th
								class="text-left py-3 px-4 font-medium text-gray-700 dark:text-gray-300 col-actions"
							>
										...
										<div class="flex flex-col">
											...
											<div class="flex items-center gap-1 mt-1">
												...
												<button
													...
													disabled={isProcessing(dataSource) ||
														(dataSource.sync_status !== 'synced' &&
															dataSource.sync_status !== 'error' &&
															dataSource.sync_status !== 'incomplete' &&
															dataSource.sync_status !== 'ingesting' &&
															dataSource.sync_status !== 'embedding' &&
															dataSource.sync_status !== 'deleted')}
													on:click={() => {
								...
								<td class="py-3 px-4 col-status h-20">
									<div class="space-y-2">
										...
										<div class="flex items-center gap-2">
											...
											{#if (dataSource.sync_status === 'deleted' || dataSource.sync_status === 'synced') && dataSource.sync_results?.delete_results}
												...
												<div class="relative group ml-1">
													<svg
														class="w-3 h-3 {hasDeleteErrors
															? 'text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300'
															: 'text-blue-500 hover:text-blue-600 dark:text-blue-400 dark:hover:text-blue-300'} cursor-help"
														fill="currentColor"
														viewBox="0 0 20 20"
													>
														<path
											...
											{#if hasSyncErrors(dataSource)}
												<div class="relative group">
													<svg
														class="w-4 h-4 text-yellow-500 dark:text-yellow-400 cursor-help"
														fill="currentColor"
														viewBox="0 0 20 20"
													>
														<path
													...
													<div
														class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 text-white text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-10"
													>
														{#if getSyncError(dataSource, 'ingesting')}
															<div class="text-xs font-medium mb-1 text-yellow-300">
																Ingestion Issues:
															</div>
															...
															<div class="text-xs text-gray-400">
																{formatErrorTimestamp(ingestingError.timestamp)}
															</div>
														{/if}
														{#if getSyncError(dataSource, 'embedding')}
															...
															<div class="text-xs text-gray-400">
																{formatErrorTimestamp(embeddingError.timestamp)}
															</div>
														{/if}
													</div>
												</div>
										...
										{:else if dataSource.sync_status === 'embedding'}
											<div class="text-xs text-gray-500 dark:text-gray-400">vectorizing data</div>
										...
										{#if dataSource.sync_status === 'deleting'}
											<div class="text-xs text-gray-500 dark:text-gray-400">deleting data...</div>
										...
										{:else if dataSource.sync_status === 'unsynced'}
											<div class="text-xs text-gray-500 dark:text-gray-400">never synced</div>
										{/if}
									</div>
								</td>
								{#if dataSource.sync_status === 'syncing' || dataSource.sync_status === 'embedding'}
				...
				{#each filteredItems as dataSource}
					...
					>
						...
						<div class="space-y-2">
							...
							<div class="flex items-center gap-2">
								<span
									class="text-xs font-medium uppercase {getSyncStatusColor(dataSource.sync_status)}"
								>
									{#if dataSource.sync_status === 'syncing'}
										{@const key = `${dataSource.action}-${dataSource.layer}`}
										{@const progressData = syncProgress[key]}
							...
							{#if dataSource.sync_status === 'syncing' || dataSource.sync_status === 'embedding'}
								<div class="space-y-2">
									...
									<!-- TODO: May restore embedding phase in future - keep all state management intact -->
									<button
										class="px-3 py-1 text-xs rounded whitespace-nowrap {getActiveView(
											dataSource
										) === 'embedding'
										...
										on:click={() =>
											setActiveView(
												dataSource,
												getActiveView(dataSource) === 'sync' ? 'embedding' : 'sync'
											)}
									>

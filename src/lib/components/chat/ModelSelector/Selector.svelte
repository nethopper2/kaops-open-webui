	...
	>
		<slot>
          ...
          <div class="absolute left-0 w-full border-t-[0.5px] mb-8 border-neutral-300 dark:border-neutral-700" />
					...
					>
						<button
							class="flex w-full font-medium line-clamp-1 select-none items-center rounded-button py-2 pl-3 pr-1.5 text-sm text-gray-700 dark:text-gray-100 outline-hidden transition-all duration-75 rounded-xl cursor-pointer data-highlighted:bg-muted hover:bg-red-100/50 dark:hover:bg-red-800/20"
							on:click={() => {
								pullModelHandler();
							}}
						>

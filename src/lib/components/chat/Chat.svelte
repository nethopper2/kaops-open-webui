...
// NOTE: This is currently only used for a specific use case: proxy-download links
async function handleGotoClick(e: MouseEvent) {
	...
	try {
		...
		// Only handle proxy-download endpoints from this handler.
		// Expected form: /files/{filePath}/proxy-download
		// Ignore other links so normal navigation or other handlers can take over.
		try {
			const pathname = targetUrl.pathname || '';
			if (!pathname.endsWith('/proxy-download')) {
				// Not a proxy-download link — do nothing.
				return;
			}
		} catch (e) {
			// If anything goes wrong inspecting the path, bail out safely.
			return;
		}
...
onMount(() => {
	...
	const handler = async ({ prevModelId, modelId, canShowPrivateAiToolbar }: {
	...
	(async () => {
		await handler({
			prevModelId: null,
			modelId: get(currentSelectedModelId),
			canShowPrivateAiToolbar: get(canShowPrivateAiSidekick)
		});
	})();
		...
		if (message) {
			...
			if (type === 'status') {
				...
				const cancelledStatus = {
					state: 'cancelled',
					description: $i18n.t('Chat stopped by user'),
					timestamp: Date.now()
				};
				if (message?.statusHistory) {
...
const sendMessageSocket = async (model, _messages, _history, responseMessageId, _chatId, privateAi?: {

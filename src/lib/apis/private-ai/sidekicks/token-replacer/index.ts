...
// In-flight request coalescing to prevent duplicate REST calls per chat+model
const valuesInFlight = new Map<string, Promise<TokenReplacementValuesResponse>>();
function keyForValues(chatId?: string | null, modelId?: string | null) {
  return `${chatId || ''}|${modelId || ''}`;
}

export async function getTokenReplacementValues(chatId: string, modelId: string): Promise<TokenReplacementValuesResponse> {
  const key = keyForValues(chatId, modelId);
  const existing = valuesInFlight.get(key);
  ...
  const p = (async () => {
    try {
      ...
      valuesInFlight.delete(key);
    }
  })();

  valuesInFlight.set(key, p);
  return p;
}
